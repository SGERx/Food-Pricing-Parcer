...
def flask_price_bill():
    logger.info("Запуск функции {func}", func="flask_price_bill")
    ...
    if len(check_data) == 0:
        ...
        logger.info("Выполнение SQL-запроса для сбора данных о ценах по чеку")
        cursor.execute(f'''
        WITH min_prices AS (
        SELECT
        shop,
        ...
        cursor.close()
        logger.info("Завершение функции {func}", func="flask_price_bill")
        return data
