...
if __name__ == '__main__':
    logger.info("Запуск файла {file} через __main__", file="test_data.py")
    ...
    logger.info("Запись тестовых данных")
    ...
    logger.info("Подтверждение внесенных в БД изменений")
    session.commit()
    logger.info("Закрытие сессии")
    session.close()
    logger.info("Завершение файла {file} через __main__", file="test_data.py")
